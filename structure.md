...
在上游仓库中添加一个临时目录本地拷贝文件，例如 `ìmported-docs`，里面每个下游具有自己的目录 `tektoncd-pipelines` 内容以下游 `docs` 目录，根据每个文件的标记决定是否同步。在上游的代码仓库中添加 `imported-docs` 到 `.gitignore`中。根据配置文件添加对应的目录链接到 `docs/*` 下
